...
var availableArgs = [
	...
	{ description: 'Release date.', name: 'date', type: 'string' },
	{
		alias: 'd',
	...
	{ description: 'Directories to ignore.', name: 'ignore', type: 'string[]' },
	{
		alias: 'h',
	...
	{ description: 'Release version.', name: 'release', type: 'string' },
	{
		alternates: ['repo'],
...
function getHelpText(availableArgs2) {
	return [
		`Usage
	$ release-bump <options>`,
		'Options' +
			availableArgs2.reduce((output, current) => {
...
function getVersionText(env) {
	return env != null && env.RELEASE_BUMP_VERSION
		? 'v' + env.RELEASE_BUMP_VERSION
		: 'no version found'
}
function parseOptionsFromArgs(passedArgs, availableArgs2) {
	return passedArgs.reduce((all, current, index) => {
		let modified = {}
		if (current.indexOf('--') === 0) {
			let [key, value] = current.substr(2).split('='),
				arg = availableArgs2.find((availableArg) => {
					var _a
					return (
						availableArg.name === key ||
						((_a = availableArg == null ? void 0 : availableArg.alternates) !=
						null
							? _a
							: []
						).includes(key)
			...
			if (arg)
				switch (arg.type) {
					case 'boolean':
						...
						break
					case 'string[]':
						modified[arg.name] = value == null ? void 0 : value.split(',')
						break
					default:
			...
			[...current.substr(1)].forEach((alias) => {
				let arg = availableArgs2.find(
					(availableArg) => availableArg.alias === alias,
				)
				arg && (modified[arg.name] = !0)
			})
		else {
			let keys = Object.keys(all),
				key = keys[keys.length - 1],
				arg = availableArgs2.find((availableArg) => availableArg.name === key)
			arg &&
				(all[key] === `$${index - 1}` || typeof all[key] > 'u') &&
				(modified[key] = arg.type === 'string[]' ? current.split(',') : current)
		}
		return __spreadValues(__spreadValues({}, all), modified)
	}, {})
}
;(async function () {
	var _a, _b
	let passedArgs =
			(_b = (_a = process.argv) == null ? void 0 : _a.slice(2)) != null
				? _b
				: [],
		_c = parseOptionsFromArgs(passedArgs, availableArgs),
		{ help, version } = _c,
		options = __objRest(_c, ['help', 'version'])
	if (help === !0) return console.info(getHelpText(availableArgs))
	if (version === !0)
		return console.info(getVersionText(define_process_env_default))
	await (0, import_index.releaseBump)(options)
})()
